...
describe('parseBatchRequest', () => {
  it('ファイルなしは 400', () => {
    const req = baseReq();
    req.files = [] as any;
    expect(() => parseBatchRequest(req as any, availability as any)).toThrow(HttpError);
    try {
      parseBatchRequest(req as any, availability as any);
    } catch (err) {
  ...
  it('妥当な入力をパースできる', () => {
    ...
    const parsed = parseBatchRequest(req as any, availability as any);
